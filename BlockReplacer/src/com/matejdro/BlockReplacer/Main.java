package com.matejdro.BlockReplacer;
...
public class Main
{
    ...
    private static HashSet<Integer> sourceBlockIDs;
    private static int targetBlockID;

    public static void main(String[] args)
    {
        if (args.length < 4)
        {
            System.out.println("java -jar BlockReplacer.jar <region folder> <minY> <maxY> <original blocks ID> <replacement block ID>");
            return;
        }
        ...
        String sourceBlockList = args[3];
        targetBlockID = Integer.parseInt(args[4]);

        sourceBlockIDs = new HashSet<>();
        String[] splitBlockIDs = sourceBlockList.split(",");
        for (String id : splitBlockIDs)
            sourceBlockIDs.add(Integer.parseInt(id));
                    ...
                    for (int i = 0; i < sectionsTag.size(); i++)
                    {
                        ...
                        for (int j = 0; j < blocksArray.length; j++)
                        {
                           ...
                           // int test = blockY * 16 * 16 + blockZ * 16 + blockX;
                            ...
                            if (blockY >= minY && blockY <= maxY && sourceBlockIDs.contains(Byte.toUnsignedInt(blocksArray[j])))
                            {
                                blocksArray[j] = (byte) targetBlockID;
                                sectionChanged = true;
                            }
                        }
